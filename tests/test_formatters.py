...
class FormattersTest(unittest.TestCase):
    ...
    def test_user_label_formatters_escape_and_fallback(self) -> None:
        ...
        self.assertEqual(
            render_user_label("7", "<Mat & Co>"), "&lt;Mat &amp; Co&gt; (user_id: 7)"
        )
    ...
    def test_alert_message_formatters_include_routes_and_thresholds(self) -> None:
        ...
        recovered = format_traffic_alert(
            {
                "user_id": 1,
                "name": "Alice",
                "period_label": "近 1 小时",
    ...
    def test_traffic_title_and_collector_alerts(self) -> None:
        self.assertEqual(
            traffic_title_for_dimension("近 1 小时", "users"),
            "📈 近 1 小时 用户流量统计",
        )
        self.assertEqual(
            traffic_title_for_dimension("近 1 小时", "nodes"),
            "📈 近 1 小时 节点流量统计",
        )
        self.assertEqual(
            traffic_title_for_dimension("近 1 小时", "combined"),
            "📈 近 1 小时 流量统计",
        )

        admin_alert = format_collector_health_alert(
            "mysql",
            recovered=False,
            detail="host=example.com port=3306",
            admin_view=True,
        )
        ...
        user_alert = format_collector_health_alert(
            "mysql",
            recovered=True,
            detail="host=example.com port=3306",
            admin_view=False,
    ...
    def test_redact_sensitive_text_for_non_admin(self) -> None:
        redacted = redact_sensitive_text_for_non_admin(
            "mysql://user:pass@example.com:3306 host=10.0.0.1 port=6379 访问 https://1.2.3.4/path"
        )
