...
class MenusTest(unittest.TestCase):
    ...
    def test_back_close_row_defaults_and_override(self) -> None:
        self.assertEqual(
            [button.callback_data for button in back_close_row()],
    ...
    def test_menu_keyboards_return_markup_with_expected_callbacks(self) -> None:
        cases = [
            (
                clear_history_confirm_keyboard(),
                {"main_menu:clear_history_confirm", "main_menu"},
            ),
            (empty_section_keyboard(), {"main_menu", "close_message"}),
            (
                health_check_keyboard(),
                {"main_menu:system_check_refresh", "main_menu", "close_message"},
            ),
            (
                traffic_management_keyboard(),
                {
                    "main_menu:traffic_users",
                    "main_menu:traffic_nodes",
                    "alert_menu:traffic",
                },
            ),
            (
...
class KeyboardsTest(unittest.TestCase):
    ...
    def test_ip_and_traffic_dashboard_callbacks(self) -> None:
        ...
        self.assertRegex(
            callbacks[0], r"^ip_active_user_detail:iprange_\d+_\d+:12:0:alert$"
        )
        self.assertEqual(callbacks[1], "alert_user:ip:12:alert")
        self.assertRegex(
            callbacks[2], r"^ip_ignore_page:area:iprange_\d+_\d+:12:0:0:alert$"
        )

        self.assertEqual(
            callback_data(
                traffic_dashboard_keyboard_static("traffic_1h", is_pinned=True)
            ),
            [
                "traffic_menu:traffic_1h",
                "traffic_dashboard:unpin:traffic_1h",
                "traffic_dashboard:delete:traffic_1h",
            ],
        )
    ...
    def test_alert_keyboards_callbacks(self) -> None:
        self.assertEqual(
            callback_data(alert_menu_keyboard("traffic")),
            [
                "alert_global:traffic",
                "alert_users:traffic:0",
                "main_menu:traffic_management",
                "close_message",
            ],
        )
        self.assertEqual(
            callback_data(alert_menu_keyboard("ip")),
            [
                "alert_global:ip",
                "alert_users:ip:0",
        ...
        self.assertIn(
            "alert_set:ip:period:24h:99",
            callback_data(alert_user_period_select_keyboard("ip", 99)),
        )
    ...
    def test_list_and_pagination_keyboards(self) -> None:
        ...
        alert_callbacks = callback_data(
            alert_user_list_keyboard(
                "traffic", [{"user_id": i, "name": f"U{i}"} for i in range(12)], page=0
            )
        )
    ...
    def test_period_and_detail_keyboards_callbacks(self) -> None:
        ...
        self.assertNotIn(
            "active_users:24h", callback_data(ip_monitor_period_result_keyboard("24h"))
        )
        self.assertEqual(
            callback_data(detail_keyboard("7d")), ["detail_back:7d", "close_message"]
        )
        self.assertEqual(
            callback_data(detail_keyboard("unexpected")),
            ["detail_back:menu", "close_message"],
        )

    def test_user_detail_and_traffic_period_callbacks(self) -> None:
        callbacks = callback_data(
            user_ip_detail_keyboard("ip_7d", 42, total_ips=11, page=0, source="alert")
        )
        ...
        query_callbacks = callback_data(
            user_ip_query_page_keyboard("30d", 42, total_ips=11, page=1)
        )
        self.assertIn("user_ip_page:42:0:30d", query_callbacks)
        self.assertIn("detail_back:30d", query_callbacks)

        period_callbacks = callback_data(
            traffic_period_keyboard("users", source_kind="traffic_1h")
        )
        ...
        self.assertTrue(
            all(
                re.match(r"^[a-z_]+(?::[A-Za-z0-9_]+)+$|^close_message$", callback)
                for callback in period_callbacks
            )
        )
