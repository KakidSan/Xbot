...
class OperationLogsTest(unittest.TestCase):
    ...
    def test_xboard_user_ids_to_labels_rewrites_only_user_references(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                xboard_user_ids_to_labels(cache_path, "用户 12"), "Alice (user_id: 12)"
            )
            self.assertEqual(
                xboard_user_ids_to_labels(cache_path, "对象：XBoard 用户 34"),
                "对象：Bob (user_id: 34)",
            )
            self.assertEqual(
                xboard_user_ids_to_labels(
                    cache_path, "邮箱 user12@example.com 与 用户 12"
                ),
                "邮箱 user12@example.com 与 Alice (user_id: 12)",
            )

    def test_operation_log_detail_display_text_converts_auth_and_xboard_labels(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            cache_path = Path(tmpdir) / "xbot.sqlite3"
            ...
            auth_detail = operation_log_detail_display_text(
                cache_path, "auth", "修改前：100\n修改后：空"
            )
            self.assertEqual(auth_detail, "修改前：Mat (@mat)\n修改后：空")

            ip_detail = operation_log_detail_display_text(
                cache_path, "ip_ignore", "对象：用户 12\n修改前：未忽略"
            )
