...
async def send_user_alert_to_chats(
    app: Application,
    chat_ids: list[str],
    text: str,
...
async def send_user_alert(
    ...
    await send_user_alert_to_chats(
        app,
        alert_notification_chats_sync(cache_path, cfg, alert_type),
        text,
        reply_markup,
    )
...
async def check_traffic_alerts(
    app: Application, cfg: AppConfig, cache_path: Path
) -> None:
    rows = traffic_alert_rows_sync(cache_path)
    current = {int(row["user_id"]): row for row in rows}
    previous_raw = (
        alert_state_get_sync(cache_path, "traffic_alert_active_users") or "[]"
    )
    try:
    ...
    for user_id, row in current.items():
        if user_id not in previous:
            await send_user_alert(
                app, cfg, cache_path, "traffic", format_traffic_alert(row)
            )
    for user_id in sorted(previous - set(current)):
        period, period_label, threshold, rule_type = (
            alert_effective_rule_detail_for_user_sync(cache_path, "traffic", user_id)
        )
        total = current_traffic_alert_value_for_user_sync(cache_path, user_id, period)
        row = {
            "user_id": user_id,
            "name": cached_user_name_by_id(cache_path, user_id) or f"用户{user_id}",
            "total": total,
            "threshold": threshold,
            "period": period,
        ...
        await send_user_alert(
            app, cfg, cache_path, "traffic", format_traffic_alert(row, recovered=True)
        )
    alert_state_set_sync(
        cache_path, "traffic_alert_active_users", json.dumps(sorted(current))
    )
...
async def check_ip_alerts(app: Application, cfg: AppConfig, cache_path: Path) -> None:
    ...
    if init_status.get("initializing"):
        log.info(
            ...
            init_status.get("active_ips"),
            init_status.get("geo_pending"),
        )
        return
    ...
    basic_or_advanced_chats = [
        chat_id for chat_id, mode in chat_modes.items() if mode in {"basic", "advanced"}
    ]
    advanced_chats = [
        chat_id for chat_id, mode in chat_modes.items() if mode == "advanced"
    ]
    for user_id, row in current.items():
        ...
        if previous_count is None:
            ...
            await send_user_alert_to_chats(
                app,
                advanced_chats,
                format_ip_alert(row, previous_city_count=previous_count),
                ip_alert_keyboard(row),
            )
    for user_id in sorted(set(previous) - set(current)):
        period, period_label, threshold, rule_type = (
            alert_effective_rule_detail_for_user_sync(cache_path, "ip", user_id)
        )
        city_count, cities = current_ip_alert_detail_for_user_sync(
            cache_path, user_id, period
        )
        row = {
            "user_id": user_id,
            "name": cached_user_name_by_id(cache_path, user_id) or f"用户{user_id}",
        ...
        await send_user_alert_to_chats(
            ...
            basic_or_advanced_chats,
            format_ip_alert(
                row, recovered=True, previous_city_count=previous.get(user_id)
            ),
        )
    alert_state_set_sync(
        cache_path,
        ...
        json.dumps(
            {
                str(user_id): int(row.get("city_count") or 0)
                for user_id, row in current.items()
            },
            sort_keys=True,
        ),
