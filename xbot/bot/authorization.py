...
def telegram_authorization_list_text_sync(cfg: AppConfig, cache_path) -> str:
    ...
    for uid in super_admin_ids:
        lines.append(f"• 👑 <b>{html.escape(telegram_user_label_sync(cache_path, uid))}</b> (<code>{uid}</code>)")
    for uid in sorted(cfg.telegram.manager_user_ids):
        lines.append(f"• 👑 {html.escape(telegram_user_label_sync(cache_path, uid))} (<code>{uid}</code>)")
...
def authorization_delete_keyboard(cfg: AppConfig, cache_path, context: ContextTypes.DEFAULT_TYPE, super_admin: bool = False) -> InlineKeyboardMarkup:
...
def authorization_role_change_text(cfg: AppConfig, cache_path, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    for uid, role in sorted(role_changes.items(), key=lambda item: int(item[0])):
        ...
        pending_lines.append(f"{emoji} {html.escape(telegram_user_label_sync(cache_path, int(uid)))} (<code>{int(uid)}</code>) → {role_label}")
    if pending_lines:
        lines.extend(["", "待保存变更：", *pending_lines])
    return "\n".join(lines)


def authorization_role_change_keyboard(cfg: AppConfig, cache_path, context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    role_changes = context.user_data.get("auth_role_changes") or {}
    if not isinstance(role_changes, dict):
        role_changes = {}
    ...
    for uid, current_role in candidates:
        ...
        emoji = "👑" if target_role == "manager" else "🎩"
        rows.append([InlineKeyboardButton(f"{emoji} {telegram_user_label_sync(cache_path, uid)} ({uid})"[:64], callback_data=f"main_menu:auth:role_toggle:{uid}")])
    if candidates:
