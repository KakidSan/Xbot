...
MAIN_MENU_PATTERN = r"^main_menu(?::(?:init_ack|clear_history|clear_history_confirm|system_check|system_check_refresh|status_notice|traffic_management|traffic_users|traffic_nodes|traffic_alerts))?$"
...
MAIN_MENU_NOTIFICATIONS_PATTERN = r"^(?:main_menu:notifications|notify)(?::(?:daily|weekly|monthly|collector|traffic_alert|ip_alert|version_update))?$"
...
def normalize_main_menu_callback(data: str) -> str:
    """Map newer callback namespaces to the legacy main_menu names handlers use.

    Router patterns accept both forms during the compatibility window. Handlers can
    keep their existing branch logic while menus gradually move to smaller
    callback namespaces.
    """
    if data == "debug:tools":
        return "main_menu:debug_tools"
    if data.startswith("auth"):
        return "main_menu:auth" + data[len("auth") :]
    if data.startswith("ip_monitor"):
        return "main_menu:ip_monitor" + data[len("ip_monitor") :]
    if data.startswith("params"):
        return "main_menu:parameter_config" + data[len("params") :]
    if data.startswith("notify"):
        return "main_menu:notifications" + data[len("notify") :]
    if data.startswith("debug"):
        return "main_menu:debug" + data[len("debug") :]
    return data
