...
from ..keyboards import (
    ...
    alert_global_period_select_keyboard,
    alert_menu_keyboard,
    ...
    *,
    ...
    if menu_match:
        ...
        await show_callback_page(
            query, text, alert_menu_keyboard(alert_type), parse_mode="HTML"
        )
        return None
    ...
    if users_match:
        ...
        users = await asyncio.to_thread(
            alert_user_list_sync, cache_path, alert_type, 10000
        )
        ...
        if not users:
            ...
            await show_callback_page(
                ...
                InlineKeyboardMarkup(
                    [back_close_row(f"alert_menu:{alert_type}", "⬅️ 返回")]
                ),
                parse_mode="HTML",
            )
            return
    ...
    if user_match:
        ...
        await show_callback_page(
            ...
            text,
            alert_user_setting_keyboard_for_source(
                bot_ctx.cache_path, alert_type, xboard_user_id, source
            ),
            parse_mode="HTML",
    ...
    global_period_page_match = re.fullmatch(
        r"alert_global_period_page:(traffic|ip)", data
    )
    if global_period_page_match:
        ...
        await show_callback_page(
            query,
            f"🕒 <b>{title}</b>\n────────────\n请选择默认告警统计周期。",
            alert_global_period_select_keyboard(alert_type),
            parse_mode="HTML",
        )
        return None
    ...
    if global_match:
        ...
        if data.endswith(":custom"):
            user_data_of(context)["awaiting_alert_global_custom"] = {
                "type": alert_type,
        ...
        await show_callback_page(
            query,
            text,
            alert_global_keyboard(bot_ctx.cache_path, alert_type),
            parse_mode="HTML",
        )
        return None

    global_period_match = re.fullmatch(
        r"alert_global:(traffic|ip):period:(1h|24h|7d|today|week)", data
    )
    if global_period_match:
        ...
        await asyncio.to_thread(
            alert_set_global_period_sync, cache_path, alert_type, period
        )
    ...
    user_period_match = re.fullmatch(
        r"alert_set:(traffic|ip):period:(1h|24h|7d|today|week):(\d+)", data
    )
    ...
    if custom_match:
        ...
        user_data_of(context)["awaiting_alert_custom"] = {
            "type": alert_type,
        ...
        await show_callback_page(
            ...
            f"✍️ 请输入独立规则 ({unit})",
            InlineKeyboardMarkup(
                [back_close_row(f"alert_user:{alert_type}:{xboard_user_id}", "⬅️ 返回")]
            ),
        )
        return None

    threshold_match = re.fullmatch(
        r"alert_set:(traffic|ip):threshold:(\d+):(\d+)", data
    )
    if threshold_match:
    ...
    if white_match:
        ...
        await asyncio.to_thread(
            ...
            "切换白名单",
            alert_setting_before_after_detail(
                alert_type, "白名单", before, after, xboard_user_id
            ),
        )
        text = await asyncio.to_thread(
            alert_user_setting_text_sync, cache_path, alert_type, xboard_user_id
    ...
    if reset_match:
        ...
        before = alert_setting_label(before_setting, alert_type, cache_path)
        await asyncio.to_thread(
            alert_reset_setting_sync, cache_path, xboard_user_id, alert_type
        )
        after_setting = await asyncio.to_thread(
            alert_user_setting_sync, cache_path, xboard_user_id
        )
