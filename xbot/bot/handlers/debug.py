    ...
    *,
    ...
    if data == "main_menu:debug:reset_cache_now_confirm":
        ...
        await asyncio.to_thread(
            ...
            "reset_cache",
            "全部重置缓存",
            f"流量样本 {stats['traffic_delta_samples']} 条，活跃 IP {stats['active_ip_records']} 条",
        )
        ...
        await show_callback_page(
            ...
            InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "🩺 前往健康检查", callback_data="main_menu:system_check"
                        )
                    ],
    ...
    if reset_user_ip_page_match:
        ...
        await asyncio.to_thread(upsert_all_cache_users, cache_path, cfg.mysql)
        users = await asyncio.to_thread(list_all_cached_user_buttons_sync, cache_path)
        selected = user_data_of(context).setdefault("reset_user_ip_selected", set())
        if not isinstance(selected, set):
            selected = set(selected or [])
            user_data_of(context)["reset_user_ip_selected"] = selected
        await answer_callback_silently(query)
        await show_callback_page(
            query,
            "👤 重置特定用户 IP 记录\n\n请选择要清理 IP 记录的用户；可多选。\n该操作会把所选用户相关的本地 IP 记录标记为忽略，不会修改 XBoard / MySQL。",
    ...
    if data == "main_menu:debug:reset_user_ip_done":
        ...
        preview = await asyncio.to_thread(
            preview_clear_user_ip_records_multi_sync, cache_path, list(selected)
        )
        label_lines = "\n".join(
            f"• {html.escape(label)}" for label in preview.get("labels", [])[:20]
        )
    ...
    if data == "main_menu:debug:reset_user_ip_multi_confirm":
        ...
        stats = await asyncio.to_thread(
            clear_user_ip_records_multi_sync, cache_path, user_ids
        )
        await asyncio.to_thread(
            log_operation_from_query_with_cache,
            bot_ctx.cache_path,
            ...
            "reset_ip",
            "重置特定用户 IP 记录",
            f"用户 {', '.join(str(uid) for uid in user_ids)}；记录 {stats['records']} 条",
        )
        user_data_of(context).pop("reset_user_ip_selected", None)
