    ...
    *,
    ...
    if data == "main_menu:op_logs":
        ...
        keyboard = await asyncio.to_thread(
            operation_logs_menu_keyboard, bot_ctx.cache_path, query.from_user.id
        )
    ...
    detail_match = re.fullmatch(
        r"main_menu:op_logs:(traffic_alert|ip_alert|ip_ignore|reset_cache|reset_ip|auth):(\d+)",
        data,
    )
    if detail_match:
        ...
        await asyncio.to_thread(
            operation_log_mark_read_sync, cache_path, query.from_user.id, log_id
        )
        await query.answer("已标记为已读")
        await show_callback_page(
            query,
            await asyncio.to_thread(
                operation_log_detail_text_sync, bot_ctx.cache_path, log_id
            ),
            operation_log_detail_keyboard(category),
            parse_mode="HTML",
        )
        return None
    log_match = re.fullmatch(
        r"main_menu:op_logs:(traffic_alert|ip_alert|ip_ignore|reset_cache|reset_ip|auth)",
        data,
    )
    if log_match:
