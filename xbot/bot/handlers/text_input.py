...
async def handle_fallback_message(
    ...
    *,
    ...
    context_bot_delete_message,
    edit_global_alert_prompt,
    edit_alert_prompt,
) -> None:
    ...
    async def reply_and_track(text: str, **kwargs: Any) -> None:
        sent = await effective_message.reply_text(text, **kwargs)
        await track_auto_delete_message(sent)

    if user_data_of(context).get("awaiting_auth_add_user_id"):
    ...
    if user_data_of(context).get("awaiting_custom_cover"):
        if not is_allowed(update, cfg):
            ...
            await reply_connection_status(update, cfg)
            return
        photos: list[Any] = list(effective_message.photo or [])
        if not photos:
        ...
        await asyncio.to_thread(
            ui_pref_set_sync, cache_path, uid, "cover_file_id", file_id
        )
        user_data_of(context).pop("awaiting_custom_cover", None)
        await reply_and_track(
    ...
    if user_data_of(context).get("awaiting_custom_nickname"):
        if not is_allowed(update, cfg):
            if is_bot_self_update(update, cfg):
                return
    ...
    if user_data_of(context).get("awaiting_alert_global_custom"):
        if not is_allowed(update, cfg):
            if is_bot_self_update(update, cfg):
                return
            user_data_of(context).pop("awaiting_alert_global_custom", None)
            await reply_connection_status(update, cfg)
            return
        custom = user_data_of(context).get("awaiting_alert_global_custom")
        ...
        async def edit_global_alert_prompt(
            message_text: str,
            ...
            try:
                await effective_message.edit_text(
                    message_text, parse_mode=parse_mode, reply_markup=keyboard
                )
            except BadRequest as exc:
        ...
        if not re.fullmatch(r"\d+", text):
            ...
            await edit_global_alert_prompt(
                f"✍️ 请输入默认规则 ({unit})\n\n⚠️ 默认规则必须是正整数，请重新输入。",
                InlineKeyboardMarkup(
                    [back_close_row(f"alert_global:{alert_type}", "⬅️ 返回")]
                ),
                None,
            )
            return
        ...
        if alert_type not in {"traffic", "ip"}:
            await edit_global_alert_prompt(
                "设置类型无效，请从菜单重新进入。", None, None
            )
            return
        input_chat_id = effective_message.chat_id
        input_message_id = effective_message.message_id
        before_period = await asyncio.to_thread(
            alert_global_period_sync, cache_path, alert_type
        )
        before_threshold = await asyncio.to_thread(
            alert_global_threshold_sync, cache_path, alert_type
        )
        ...
        await asyncio.to_thread(
            alert_set_global_threshold_sync, cache_path, alert_type, value
        )
        after_threshold = await asyncio.to_thread(
            alert_global_threshold_sync, cache_path, alert_type
        )
        ...
        await asyncio.to_thread(
            ...
            "调整默认规则",
            alert_setting_before_after_detail(alert_type, "默认规则", before, after),
        )
        result = await asyncio.to_thread(
            alert_global_setting_text_sync, cache_path, alert_type
        )
        await edit_global_alert_prompt(
            result, alert_global_keyboard(bot_ctx.cache_path, alert_type), "HTML"
        )
        await context_bot_delete_message(input_chat_id, input_message_id)
        return None

    if user_data_of(context).get("awaiting_alert_custom"):
        if not is_allowed(update, cfg):
            if is_bot_self_update(update, cfg):
                return
            user_data_of(context).pop("awaiting_alert_custom", None)
            await reply_connection_status(update, cfg)
            return
        custom = user_data_of(context).get("awaiting_alert_custom") or {}
        alert_type = str(custom.get("type") or "")
        ...
        if not re.fullmatch(r"\d+", text):
            ...
            await edit_alert_prompt(
                ...
                InlineKeyboardMarkup(
                    [
                        back_close_row(
                            f"alert_user:{alert_type}:{xboard_user_id}", "⬅️ 返回"
                        )
                    ]
                ),
                None,
        ...
        input_chat_id = effective_message.chat_id
        input_message_id = effective_message.message_id
        before_setting = await asyncio.to_thread(
            alert_user_setting_sync, cache_path, xboard_user_id
        )
        ...
        if alert_type == "traffic":
            await asyncio.to_thread(
                ...
                cache_path,
                xboard_user_id,
                traffic_threshold_bytes=value * 1024**3,
                traffic_whitelist=0,
            )
        elif alert_type == "ip":
            await asyncio.to_thread(
                ...
                cache_path,
                xboard_user_id,
                ip_city_threshold=value,
                ip_whitelist=0,
            )
        else:
            await edit_alert_prompt("设置类型无效，请从菜单重新进入。", None, None)
            return
        after_setting = await asyncio.to_thread(
            alert_user_setting_sync, cache_path, xboard_user_id
        )
        ...
        await asyncio.to_thread(
            ...
            "调整独立规则",
            alert_setting_before_after_detail(
                alert_type, "独立规则", before, after, xboard_user_id
            ),
        )
        result = await asyncio.to_thread(
            alert_user_setting_text_sync, cache_path, alert_type, xboard_user_id
        )
        await edit_alert_prompt(
            result,
            alert_user_setting_keyboard(bot_ctx.cache_path, alert_type, xboard_user_id),
            "HTML",
        )
        await context_bot_delete_message(input_chat_id, input_message_id)
        return None

    if user_data_of(context).get("awaiting_user_ip_query_id"):
        if not is_allowed(update, cfg):
            if is_bot_self_update(update, cfg):
                return
            user_data_of(context).pop("awaiting_user_ip_query_id", None)
        ...
        if not re.fullmatch(r"\d+", text):
            await reply_and_track(
                "用户 ID 必须是数字，请重新输入；或发送 /start 取消。"
            )
            return
        user_data_of(context).pop("awaiting_user_ip_query_id", None)
        period_key = user_data_of(context).pop("user_ip_query_period", None)
        ...
        await edit_or_replace_status(
            ...
            update,
            parse_mode="HTML",
            reply_markup=user_ip_query_page_keyboard(
                period_key, xboard_user_id, total_ips, 0
            ),
        )
        return None
