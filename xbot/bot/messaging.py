...
from .keyboards import traffic_custom_available_bounds, traffic_custom_single_year, traffic_dashboard_keyboard
...
def start_menu_text(update: Update, cfg: AppConfig, custom_name: str | None = None) -> str:
...
def mark_no_auto_delete_message(no_auto_delete_message_keys: set[tuple[str, int]], message: Any | None) -> None:
    if not message:
        return
    try:
...
async def track_auto_delete_message(cache_path: Path, message: Any | None, is_pinned: bool = False) -> None:
    if not message:
        return
    try:
        await asyncio.to_thread(auto_delete_message_set_sync, cache_path, str(message.chat_id), message.message_id, is_pinned)
...
async def reply_long_text(track_auto_delete_message_func: Any, message: Any, text: str, parse_mode: str | None = None, reply_markup: InlineKeyboardMarkup | None = None) -> None:
...
async def show_initialization_gate(cache_path: Path, cfg: AppConfig, answer_callback_silently_func: Any, show_callback_page_func: Any, query: Any) -> bool:
...
async def context_bot_delete_message(app: Any, chat_id: int | str, message_id: int) -> bool:
    try:
...
async def purge_chat_history(cache_path: Path, context_bot_delete_message_func: Any, chat_id: int | str, from_message_id: int) -> tuple[int, int]:
    deleted = 0
    failed = 0
    start_id = max(1, int(from_message_id))
    batch_size = 25
    for batch_start in range(start_id, 0, -batch_size):
        tasks = [context_bot_delete_message_func(chat_id, message_id) for message_id in range(batch_start, max(0, batch_start - batch_size), -1)]
...
async def delete_trigger_command_message(context_bot_delete_message_func: Any, update: Update) -> None:
...
async def send_start_menu(cache_path: Path, cfg: AppConfig, track_auto_delete_message_func: Any, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if custom_cover:
        try:
            sent = await update.effective_message.reply_photo(photo=custom_cover, caption=text, parse_mode="HTML", reply_markup=main_menu_keyboard(is_admin))
            await track_auto_delete_message_func(sent)
            return
        except Exception as exc:
    ...
    try:
        ...
        if photos.total_count > 0 and photos.photos:
            sent = await update.effective_message.reply_photo(photo=photos.photos[0][-1].file_id, caption=text, parse_mode="HTML", reply_markup=main_menu_keyboard(is_admin))
            await track_auto_delete_message_func(sent)
            return
    except Exception as exc:
...
async def reply_cover_card(cache_path: Path, track_auto_delete_message_func: Any, update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, reply_markup: InlineKeyboardMarkup | None = None):
    if not update.effective_message:
        return None
    uid = user_id(update)
    custom_cover = await asyncio.to_thread(ui_pref_get_sync, cache_path, uid, "cover_file_id") if uid is not None else None
    if custom_cover:
        try:
            sent = await update.effective_message.reply_photo(photo=custom_cover, caption=text, parse_mode="HTML", reply_markup=reply_markup)
            await track_auto_delete_message_func(sent)
            return sent
        except Exception as exc:
    ...
    try:
        ...
        photos = await context.bot.get_user_profile_photos(me.id, limit=1)
        if photos.total_count > 0 and photos.photos:
            sent = await update.effective_message.reply_photo(photo=photos.photos[0][-1].file_id, caption=text, parse_mode="HTML", reply_markup=reply_markup)
            await track_auto_delete_message_func(sent)
            return sent
    except Exception as exc:
...
async def reply_main_menu(cache_path: Path, cfg: AppConfig, send_start_menu_func: Any, update: Update, context: ContextTypes.DEFAULT_TYPE, _cfg: AppConfig | None = None) -> None:
    if not update.effective_message:
        return
    uid = user_id(update)
...
async def auto_delete_unpinned_dashboard(app: Any, cache_path: Path, chat_id: str, message_id: int, kind: str) -> None:
...
async def send_dashboard_card(app: Any, cache_path: Path, message: Any, kind: str, user_pref_id: int | None = None) -> None:
    ...
    asyncio.create_task(auto_delete_unpinned_dashboard(app, cache_path, chat_id, sent.message_id, kind))


async def edit_dashboard_card(cache_path: Path, show_callback_page_func: Any, query: Any, kind: str) -> None:
...
async def open_dashboard_card(send_dashboard_card_func: Any, query: Any, kind: str) -> None:
    if not query.message:
        return
...
def traffic_custom_prompt_text(cache_path: Path, state: dict[str, Any]) -> str:
    ...
    lines = [f"请选择{target_text}的{step_label}。", f"（可选择的最早时间：{format_timestamp(first_ts)}）"]
    if phase == "start":
...
async def send_or_jump_traffic_dashboard(send_dashboard_card_func: Any, message: Any, kind: str) -> None:
    sender = getattr(message, "from_user", None)
    await send_dashboard_card_func(message, kind, sender.id if sender else None)


async def open_traffic_dashboard_message(send_or_jump_traffic_dashboard_func: Any, query: Any, kind: str) -> None:
    if not query.message:
        return
...
async def switch_traffic_dashboard_message(edit_dashboard_card_func: Any, query: Any, kind: str) -> None:
    if not query.message:
        return
...
async def edit_message_prompt(context: ContextTypes.DEFAULT_TYPE, chat_id: int | str | None, message_id: int | None, message_text: str, keyboard: InlineKeyboardMarkup | None = None, parse_mode: str | None = "HTML") -> None:
...
def build_runtime_services(app: Any, cfg: AppConfig, cache_path: Path) -> dict[str, Any]:
    ...
    show_page = partial(show_callback_page, cache_path, no_auto_delete_message_keys, track)
    ...
    return {
        ...
        "delete_trigger_command_message": partial(delete_trigger_command_message, context_delete),
        ...
        "show_initialization_gate": partial(show_initialization_gate, cache_path, cfg, answer_callback_silently, show_page),
        ...
        "resolve_telegram_user_label": partial(resolve_telegram_user_label, app, cache_path),
        "mark_no_auto_delete_message": partial(mark_no_auto_delete_message, no_auto_delete_message_keys),
        "send_dashboard_card": send_dashboard,
        "edit_dashboard_card": edit_dashboard,
        "open_traffic_dashboard_message": partial(open_traffic_dashboard_message, send_or_jump),
        "switch_traffic_dashboard_message": partial(switch_traffic_dashboard_message, edit_dashboard),
