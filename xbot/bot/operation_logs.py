...
def operation_logs_menu_keyboard(
    cache_path: Path, viewer_user_id: int
) -> InlineKeyboardMarkup:
    counts = operation_log_counts_sync(
        cache_path, viewer_user_id, list(OPERATION_LOG_CATEGORIES.keys())
    )

    def button(text: str, category: str) -> InlineKeyboardButton:
        unread, total = counts.get(category, (0, 0))
        return InlineKeyboardButton(
            f"{text} ({unread}/{total})", callback_data=f"main_menu:op_logs:{category}"
        )
...
def operation_log_detail_display_text(
    cache_path: Path, category: str, detail: str
) -> str:
    if not detail:
        return detail
    converted: list[str] = []
...
def operation_logs_summary_keyboard(
    cache_path: Path, category: str, viewer_user_id: int
) -> InlineKeyboardMarkup:
    rows = []
    ...
    for row in logs:
        ...
        ts = datetime.fromtimestamp(
            int(row.get("created_at") or 0), BEIJING_TZ
        ).strftime("%m-%d")
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    label[:64], callback_data=f"main_menu:op_logs:{category}:{log_id}"
                )
            ]
        )
    if not rows:
    ...
    rows.append(
        [
            InlineKeyboardButton("⬅️ 返回操作日志", callback_data="main_menu:op_logs"),
            InlineKeyboardButton("❌ 关闭", callback_data="close_message"),
        ]
    )
    return InlineKeyboardMarkup(rows)
...
def operation_log_summary_text_sync(
    cache_path: Path, category: str, viewer_user_id: int
) -> str:
    label = OPERATION_LOG_CATEGORIES.get(category or "", "操作日志")
    unread, total = operation_log_counts_sync(
        cache_path, viewer_user_id, [category]
    ).get(category, (0, 0))
    return "\n".join(
        [
            "📜 <b>操作日志</b>",
...
def operation_log_detail_text_sync(cache_path: Path, log_id: int) -> str:
    ...
    ts = datetime.fromtimestamp(int(row.get("created_at") or 0), BEIJING_TZ).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    detail = html.escape(
        operation_log_detail_display_text(
            cache_path, category, str(row.get("detail") or "")
        )
    )
    lines = [
        "📜 <b>操作日志详情</b>",
...
def operation_log_detail_keyboard(category: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "⬅️ 返回记录列表", callback_data=f"main_menu:op_logs:{category}"
                )
            ],
            [
                InlineKeyboardButton(
                    "⬅️ 返回操作日志", callback_data="main_menu:op_logs"
                ),
                InlineKeyboardButton("❌ 关闭", callback_data="close_message"),
            ],
        ]
...
def log_operation_from_query(
    cache_path: Path, query: Any, category: str, action: str, detail: str = ""
) -> None:
    user = getattr(query, "from_user", None)
    operation_log_add_sync(
        cache_path,
        getattr(user, "id", None),
        actor_name_from_user(user),
        category,
        action,
        detail,
    )
...
def log_operation_from_update(
    cache_path: Path, update: Update, category: str, action: str, detail: str = ""
) -> None:
