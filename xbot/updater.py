...
def run_command_sync(
    args: list[str], cwd: Path = APP_DIR, timeout: int = 20
) -> tuple[int, str, str]:
    try:
        proc = subprocess.run(
            args, cwd=str(cwd), text=True, capture_output=True, timeout=timeout
        )
...
def version_keyboard(
    check: dict[str, Any] | None = None, admin_view: bool = True
) -> InlineKeyboardMarkup:
    ...
    if (
        admin_view
        and check
        and check.get("has_update")
        and VERSION_TAG_RE.fullmatch(latest)
    ):
        rows.append(
            [
                InlineKeyboardButton(
                    f"⬆️ 后台更新到 {latest}",
                    callback_data=f"version_update:start:{latest}",
                )
            ]
        )
    rows.append(
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ 确认后台更新",
                    callback_data=f"version_update:confirm:{target_version}",
                )
            ],
...
def version_notice_already_sent_sync(
    cache_path: Path, latest: str, date_text: str
) -> bool:
    return (
        get_collector_state_sync(cache_path, version_notice_sent_key(latest, date_text))
        is not None
    )


def mark_version_notice_sent_sync(
    cache_path: Path, latest: str, date_text: str
) -> None:
    ...
    while not stop_event.is_set():
        try:
            ...
            if now.hour == 12 and now.minute == 0:
                ...
                if (
                    check.get("has_update")
                    and latest
                    and not await asyncio.to_thread(
                        version_notice_already_sent_sync, cache_path, latest, date_text
                    )
                ):
                    ...
                    for chat_id in chats:
                        try:
                            admin_view = str(chat_id) in {
                                str(uid) for uid in cfg.telegram.super_admin_user_ids
                            }
                            await app.bot.send_message(
                                chat_id=chat_id,
                                text=version_update_notice_text(check),
                                parse_mode="HTML",
                                reply_markup=version_keyboard(
                                    check, admin_view=admin_view
                                ),
                            )
                        except Exception as exc:
                            log.warning(
                                "发送版本更新通知失败 chat=%s：%s", chat_id, exc
                            )
                    await asyncio.to_thread(
                        mark_version_notice_sent_sync, cache_path, latest, date_text
                    )
        except Exception as exc:
